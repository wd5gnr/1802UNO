...
void SSD1306Ascii::paintscreen(uint8_t l1, uint8_t l2)
{
  ...
  for (line=l1;line<=l2;line++)        // 8 lines because we're using line doubling
  {
    for (block=0;block<8;block++)
    {
      ...
      { 
        // A[i*2+1] = A[i*2] = pgm_read_byte_near (startrek + line*32 + block + (3-i)*(8));  // debug test version
        A[i*2+1] = A[i*2] = ram[line*32 + block + (3-i)*(8)];  // line*32 - line doubling, would be 64 otherwise. (3-i): line doubling, would be (7-i) otherwise
      }
